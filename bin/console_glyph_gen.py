...
def _print_progress(msg, verbose=False, end="\n"):
	if not print_progress_verbose and verbose: return
	if print_progress: 
		if _enable_color:
			sys.stdout.write("\033[38;2;120;220;190m" + msg + "...\033[0m" + end)
		else:
...
if list_optimize:
	if draw_graph:
		...
		for _ in range(34): print(flush=True)
		print("\033[34A", flush=True)
	
	final_charsets = list()
	for i in range(0, total_list_tries):
		_print_progress("{}List {} of {}".format("\n" + str().ljust(45, "~") + "\n" if print_progress_verbose else "", \
														str(i + 1).rjust(len(str(total_list_tries))), total_list_tries), end="\r")
		if print_progress_verbose: print(flush=True)
		...
		if draw_graph:
			...
			print(SEPPrinting.console_graph([fcs[3] for fcs in final_charsets], \
																			maxWidth=128, maxHeight=32, \
																			debug=False, center=(4, 4, 1, 0)) \
						, flush=True)
			
			
			if not print_progress_verbose:
				print("\033[36A\033[128D", flush=True)
			else:
				print("\033[43A\033[128D", flush=True)
	
	#sort by error
	final_charsets = sorted(final_charsets, key=lambda s: s[3])
	
	print("\033[58B", flush=True)
	print("\nFinal result:\n\n")
